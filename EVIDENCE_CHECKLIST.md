...
2. **Module loaded** — Studio's Modules tab or boot log showing Notion Guard `v1.0.2`, signature verified, 10 commands, `loaded=1 rejected=0`. Since the module is now `license_required: true`, this station needs a valid license file installed first (`railcall market claim <purchase_id>` or `railcall market auto-claim` after purchase) — without one, the boot log shows a rejection with a `license:` reason instead.
