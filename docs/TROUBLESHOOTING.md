...
**"This approval is N minutes old, over the 30-minute freshness limit"** — RailCall approvals never platform-expire on their own; single-use consumption is the platform's only built-in protection. All four write commands additionally check their own approval's age and refuse one older than 30 minutes, so a delayed, queued, or long-forgotten execute can't act on a stale human decision. Preview and approve again to execute now. This check only ever adds a restriction on top of the platform's own approval flow — if RailCall's own `pending_approvals.json` record can't be found or read for any reason, the write proceeds exactly as it always has, since a missing bookkeeping file is not evidence of staleness.
